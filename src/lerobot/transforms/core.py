...
class PadStateAndActionTransformFn(DataTransformFn):
    max_state_dim: int = 56      # original 32
    max_action_dim: int = 56       # original 32
...
class NormalizeTransformFn(DataTransformFn):
    ...
    def __call__(self, data: DataDict) -> DataDict:
        ...
        for key in keys:
            ...
            # Tactile force uses dataset-level max-abs normalization from stats.
            if key == "observation.tactile":
...
def hydrate_normalize_transform(
    ...
    for t in transforms:
        ...
        if isinstance(t, NormalizeTransformFn):
            ...
            # Normalize tactile force targets when present in dataset stats.
            if "observation.tactile" in dataset.meta.stats:
                selected_keys.append("observation.tactile")
            print(
