...
class PreTrainedPolicy(nn.Module, HubMixin, abc.ABC):
    ...
    def _load_as_safetensor(cls, model: T, model_file: str, map_location: str, strict: bool) -> T:
        def drop_mismatched_keys_pre_hook(*args, **kwargs):
            state_dict = None
            for x in args:
            ...
            if state_dict is None:
                logging.warning(
                    "[Checkpoint loading] Could not find state_dict in load_state_dict hook args."
                )
                return
            ...
            if to_delete:
                logging.warning(
                    "[Checkpoint loading] Dropping mismatched keys:\n  "
                    + "\n  ".join(to_delete)
                )

            for k in to_delete:
                del state_dict[k]
        model._register_load_state_dict_pre_hook(drop_mismatched_keys_pre_hook)
